...
export class RuleParser {
   ...
   * Looks for: **Rules**: followed by ```json ... ```
   */
  /**
   * Extract JSON rules from markdown content
   * Supports:
    ...
    if (linkMatch) {
      ...
      try {
        ...
        if (
          ...
          // In tests we might be outside workspace, but generally safeguard
          // Relaxing slightly for local dev/test if needed, but best to enforce
        }
   ...
   * Throws if nesting exceeds MAX_RULE_DEPTH
   */
  private validate(rules: RuleCondition, depth: number): RuleCondition {
    ...
    // Validate simple single-rule case (has pattern but no conditions)
    ...
    if (rules.conditions && Array.isArray(rules.conditions)) {
      for (const condition of rules.conditions) {
        if (isFileRule(condition)) {
          ...
          // Recursive validation for nested RuleCondition
