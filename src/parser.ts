...
export class DecisionParser {
  ...
  // Status synonyms for flexibility
  ...
  // Severity synonyms for flexibility
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          // We call parseFile recursively but we know it's a file now,
          // however calling parseFile ensures path safety checks again which is fine.
          // To avoid infinite recursion if we didn't check isDirectory, we use separate logic.
          // But here we can just read and call parseContent.
  ...
  /**
   * Split content into decision blocks using robust regex search
   * Handles malformed splitting and ensures only valid blocks are captured
   */
  ...
  private async parseBlock(
    ...
    // Extract advanced rules (JSON format)
   ...
   * Anchored to single lines with multiline flag
   */
   ...
   * Handles both with and without backticks
   */
